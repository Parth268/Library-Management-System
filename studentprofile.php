<?php 
 ...
 if(@$_REQUEST['namesearch']=='search')
  ...
  echo "<script>window.location.replace('search.php')</script>";
   ...
   $qu=$cat->getdata($con," * "," studentdata"," where enroll = '".$enroll."'","");

   //$qu=mysqli_query($con,"select * from studentdata where enroll ='$enroll';");
   if($re=mysqli_fetch_array($qu)){
     $sub=$re['book_submit1'];
     $sub2=$re['book_submit2'];
   }
		...
		<div class='cssmenu'>
			<ul> <?php 
			   echo "<li class='active'><a href='index.php'><span>Home</span></a></li>
			    ...
			    echo "<li ><a href='studentprofile.php'><span>Profile</span></a></li>";
           echo "<li><div>
...
<div class='section group'>	
    ...
    if (!@$_SESSION['stat']) {
       ...
       $query=$cat->getdata($con," * "," studentdata","where enroll = '".$enroll."'","");

   // $query=mysqli_query($con,"select * from studentdata where enroll= '$enroll' ;");
    ...
    echo "<table class='table table-striped' >";
 	if($rec=mysqli_fetch_array($query))
 	{
 		...
 		echo "<tr><td>Enroll</td><td> ".$rec['enroll']." </td></tr>";
    ...
    echo "<tr><td>Book1</td><td> ".$rec['book_code1']." </td><tr>";
    echo "</tr><td>Date</td><td>".$rec['book_issue1']." to ".$rec['book_submit1']."</td></tr>";
    echo "<tr><td>Book2</td><td> ".$rec['book_code2']." </td><tr>";
    echo "</tr><td>Date</td><td>".$rec['book_issue2']." to ".$rec['book_submit2']."</td></tr>";
    echo "<tr><td>Book1 Fine</td><td> ".$rec['book1_fine']." </td><tr>";
    echo "<tr><td>Book2 Fine</td><td> ".$rec['book2_fine']." </td><tr>";
...
echo "<table class='table table-striped' >";
 ...
 else
 {
  header("location:index.php");
    ...
    if(@$_REQUEST['studentupdate']=='done')
    {
    ...
    $dept=@$_REQUEST['dept'];
          $statues=$cat->editdata($con,"studentdata", array("name" => $name, "sem" => $sem, "surname" => $surname, "mobile" => $mobile, "dept" => $dept ) , " where enroll = '".$enroll."'");

         //$statues=mysqli_query($con,"update studentdata set name='".$name."', sem='".$sem."', surname='".$surname."', mobile='".$mobile."', dept='".$dept."' where enroll='".$enroll."';");
         if($statues){ 
                      echo "<script>alert('Record  updated');</script>";
                      echo "<script>window.location.replace('studentprofile.php')</script>";
                    }
         else{
    ...
    echo  "<a href='#' data-toggle='modal' data-target='#studentupdate' class='btn btn-info'>Update</a>";
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <div class="form-group">
        ...
        <input type="hidden" name="enroll" value="<?php echo $rec['enroll'] ?>">
        <br></div>
                <div class="form-group">
        ...
        <br></div>
         ...
         <div class="form-group">
        <input type="text" name="Semester"  required class="form-control" value="<?php echo $rec['sem'] ?>" placeholder="Semester" >
        <br></div>
                <div class="form-group">
      ...
      <div class="modal-footer" >
      	<input type="submit" class="btn btn-default"   name="studentupdate" value="done" >
